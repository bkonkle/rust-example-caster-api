...
create or replace function on_delete_show ()
    returns trigger
    as $$
begin
...
create trigger on_delete_show
    before delete on shows for each row
    execute procedure on_delete_show ();

create trigger sync_shows_updated_at
    before update on shows for each row
...
create table episodes (
    ...
    summary text,
    picture text,
    show_id text not null references shows on update cascade on delete cascade
);
...
create or replace function on_delete_episode ()
    returns trigger
    as $$
begin
    delete from role_grants
...
create trigger on_delete_episode
    ...
    execute procedure on_delete_episode ();
